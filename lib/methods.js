...
Meteor.methods({
  ...
  'rooms.autogenerateDropin'(name, destination, tags=[]) {
    return autogenerateDropin(name, destination, tags)
  },
  ...
  'rooms.addPlayer'(roomId, playerId) {
    var room = Rooms.findOne({_id: roomId})
    //console.log("add player to " + room.name)
    var players = room.currentPlayers
    if(!players) {
      players = []
    }
  ...
  'rooms.removePlayer'(roomId, playerId) {
    var room = Rooms.findOne({_id: roomId})
    //console.log("remove player from " + room.name)
    var players = room.currentPlayers
    if(!players) {
       players = []
    }
