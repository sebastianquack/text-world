...
autogenerateDropin = function(name, destination, tags) {
  var placeDescription = "You are at the " + name + ". "
  var connection = connections[Math.floor(Math.random() * connections.length)]
  placeDescription += "<br><br>There is "
  placeDescription += `a ${connection.name} to the ${destination}.`
  var moveActionsDescription = `<b>${connection.action} the ${connection.name}</b> `
  var moveActionsCode = `if(input.contains("${connection.action}", "${connection.name}", "${destination}")) {
  output("You ${connection.action} the ${connection.name} to the ${destination}...")
...
`;
  ...
  var result = Rooms.insert(object)  
  
  // research for exits on destination when creating dropin
  var destinationRoom = Rooms.findOne({slug: slugify(destination)})
  if(destinationRoom) {
    Rooms.update(destinationRoom._id, { $set: { 
      exits: findExits(destinationRoom.script)  
    }})
  } 
...
autogenerateRoom = function(tags) {
  ...
  var placeIndex = 0
  do {
    var frontMod = Math.random() < 0.3 ? frontModifiers[Math.floor(Math.random() * frontModifiers.length)] + " " : ""
    var backMod = Math.random() < 0.3 ? " " + backModifiers[Math.floor(Math.random() * backModifiers.length)] : ""
    placeIndex = Math.floor(Math.random() * placeNames.length)
    placeName = frontMod + placeNames[placeIndex] + backMod
    counter++  
  } while(Rooms.findOne({slug: slugify(placeName)}) && counter < maxTries);
  if(counter >= maxTries) {
    return null
  }
  ...
  var destinations = []
  if(tags.length > 0) {
    destinations = Rooms.find({$and: [{"tags": tags[0]}, {"visibility": {$in: ["unlisted", "public"]}}]}).fetch() 
  } else {
    destinations = Rooms.find({"visibility": {$in: ["unlisted", "public"]}}).fetch() // all rooms are possible destinations
  }
